...
class ShipCompClient {
  ...
  async checkComplianceAndCommitSalesOrder (shopifyOrder) {
    ...
    requestObj.Request.SalesOrder.Shipments.Shipment.ShipmentStatus = 'SentToFulfillment';
    requestObj.Request.SalesOrder.Shipments.Shipment.FulfillmentHouse = 'WineShippingSantaMaria';
  ...
  async voidSalesOrder (orderKey) {
    ...
    const result = {
      success: _.get(response, 'VoidSalesOrderResult.ResponseStatus') ===
        'Success',
      errors: _.get(response, 'GetSalesOrderResult.Errors') || null
    };
    return result;
