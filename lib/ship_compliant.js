...
async function loadSCMethods () {
  try {
    ...
    const mapping = {
      ...
      voidSalesOrder: p(
        salesOrderClient.SalesOrderService.SalesOrderServiceSoap12.VoidSalesOrder
      ),
    };
...
class ShipCompClient {
  ...
  async getSalesOrder (orderKey) {
    ...
    // console.log(JSON.stringify(response, null, 2));
    const packages = _.get(response, 'GetSalesOrderResult.SalesOrder.Shipments.Shipment[0].Packages.Package') || [];

    const result = {
      success: _.get(response, 'GetSalesOrderResult.ResponseStatus') === 'Success',
      status: _.get(response, 'GetSalesOrderResult.SalesOrder.Shipments.Shipment[0].ShipmentStatus'),
      tracking: packages.map(pack => pack.TrackingNumber),
      shipping_service: _.get(response, 'GetSalesOrderResult.SalesOrder.Shipments.Shipment[0].ShippingService'),
      sales_order_key: _.get(response, 'GetSalesOrderResult.SalesOrder.SalesOrderKey'),
  ...
  async voidSalesOrder (orderKey) {
    ...
    const response = await scMethods.voidSalesOrder(requestObj);
    const result = {
      success: _.get(response, 'VoidSalesOrderResult.ResponseStatus') === 'Success',
      errors: _.get(response, 'GetSalesOrderResult.Errors') || null,
    };
    return result;
  }
