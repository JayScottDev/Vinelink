...
const getStateFromCache = async (zip) => {
  ...
  return await cache.getAsync(zip);
};

const cacheResult = async (zip, state) => {
  const key = `${key_prefix}${zip}`;
  cache.setAsync(zip, state);
};
...
const getStateExternally = async (zip) => {
  ...
  // validate input zip
  const zipComponent = addrComponents.filter(comp => comp.types.some(type => type === 'postal_code'));
  const isZip = zipComponent.length ? zipComponent[0].short_name === String(zip) : false;
  if (!isZip) return null;

  // validate country
  const countryComponent = addrComponents.filter(comp => comp.types.some(type => type === 'country'));
  const isUSA = countryComponent.length ? countryComponent[0].short_name === 'US' : false;
  if (!isUSA) return null;

  // parse out state from address object
  const filteredComponents = addrComponents.filter(comp => comp.types.some(type => type === 'administrative_area_level_1'));
