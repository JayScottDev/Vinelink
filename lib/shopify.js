...
module.exports.registerWebhooks = async () => {
  ...
  for (let shop of shops) {
    await registerWebhooksByShop(shop);
  }
  console.log('Shopify Webhooks registered');
};

async function registerWebhooksByShop (shop) {
  try {
    const { webhooks: existing } = await shopifyRequest(
      shop,
      'GET',
      'admin/webhooks'
    );
    for (let webhook of webhooks) {
      ...
      if (
        !existing.some(
          ex => ex.topic === webhook.topic && ex.address === address
        )
      ) {
...
module.exports.registerWebhooksByShop = registerWebhooksByShop;
...
module.exports.syncFulfillments = async order => {
  ...
  const shopifyFulfillments = await shopifyRequest(
    shop,
    'GET',
    `admin/orders/${order.order_key}/fulfillments`
  );
  let fulfillmentId;
